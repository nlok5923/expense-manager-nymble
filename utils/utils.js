...
const weekWiseExpenditure = (weeksInMonths, transactions) => {
    ...
    weeksInMonths.map((week) => {
        ...
        transactions.map((transaction) => {
            if(new Date(transaction.date).getDay() >= startDate && new Date(transaction.date).getDay() <= endDate) {
                sum += Number(transaction.amount);
                isTransactionHappen = true;
            }
        });
        if(isTransactionHappen)
        perWeekExpenditure.push(sum); 
        else
        perWeekExpenditure.push(-1);
    })
...
const expenditureCategoryWise = (expenses) => {
    ...
    expenses.map((expense) => {
        category.set(expense.category, +category.get(expense.category) + +expense.amount);
    });
    let categoricalSum = [];
...
module.exports = { getCurrentDate, getCurrentTime, getWeeksInMonth, weekWiseExpenditure, monthlyTransactions, expenditureCategoryWise };